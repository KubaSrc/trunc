...
class aux_bot():
    ...
    def __init__(self,drive_path = None, pos_path = None, motor_path = None,
        ...
        if  train_inverse:
            ...
            self.ik_net.load_state_dict(torch.load(drive_path + '/models/inverse_2024_02_02-17_46_11_14.739pwm'))
    ...
    def eval_model_inverse(self,ik_net,loss):
        ...
        # Save model
        model_name = 'inverse_'+time.strftime('%Y_%m_%d-%H_%M_%S_') + '{:.3f}mm'.format(test_accuracy)
    ...
    def inverse_prediction(self,input_path=None,output_path=None):
        with torch.no_grad():
            ...
            # Generate motor trajectory
                ...
                self.test_sequence =   self.SequenceDataset(traj,
                                                input_range = self.motor_slice,
                                                output_range = self.end_slice,
                                                sequence_length = self.MAX_SEQ_LENGTH,
                                                shuffle = False,
